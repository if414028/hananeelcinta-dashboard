...
<header class="page-container fixed inset-x-0 top-6 z-50">
    <nav class="floating-nav flex items-center justify-between" aria-label="Navigasi utama">
        ...
        <button class="grid h-11 w-11 place-items-center rounded-full border border-ink lg:hidden" @click="menuOpen = !menuOpen" :aria-expanded="menuOpen" aria-label="Buka menu">☰</button>
    </nav>
...
<main>
    <section class="page-container flex min-h-screen items-center pb-16 pt-36">
        <div class="grid w-full items-end gap-10 lg:grid-cols-[1.3fr_.7fr]">
            ...
            <div class="col-span-full min-h-72 rounded-[40px] bg-ink p-8 text-canvas shadow-[0_24px_48px_rgba(0,0,0,0.08)] md:min-h-96 md:p-12"><div class="flex h-full max-w-xl flex-col justify-end"><p class="text-sm font-bold uppercase tracking-[0.04em] text-canvas/60">Kasih yang mengubahkan</p><h2 class="mt-4 text-4xl md:text-5xl">Berakar dalam Firman. Bertumbuh dalam komunitas.</h2></div></div>
        </div>
    </section>
...
<footer id="kontak" class="bg-ink py-20 text-white"><div class="page-container"><h2 class="max-w-2xl text-4xl">Kami selalu ada saat Anda membutuhkan keluarga untuk berjalan bersama.</h2><div class="mt-14 border-t border-white/20 pt-8 text-sm text-white/70">© {{ date('Y') }} JKI Hananeel Cinta</div></div></footer>
</body>
</html>
