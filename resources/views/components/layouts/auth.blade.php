<!doctype html>
<html lang="id">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>{{ $title ?? 'Admin' }} · JKI Hananeel Cinta</title>@vite(['resources/css/app.css', 'resources/js/app.js'])</head>
<body>
<main class="auth-shell">
    ...
    <section class="grid place-items-center px-5 py-12 sm:px-8"><div class="w-full max-w-md"><a href="{{ route('home') }}" class="mb-8 inline-flex min-h-12 items-center gap-3 rounded-full pr-4" aria-label="Kembali ke beranda"><img src="{{ Vite::asset('resources/project-assets/logo.webp') }}" alt="" class="h-14 w-14 rounded-full object-cover"><span><strong class="block text-base">JKI Hananeel Cinta</strong><span class="text-sm text-slate">Admin workspace</span></span></a>@if (session('success'))<x-alert class="mb-5">{{ session('success') }}</x-alert>@endif{{ $slot }}<p class="mt-8 text-center text-xs text-slate">Area terbatas untuk admin yang berwenang.</p></div></section>
</main>
</body>
</html>
