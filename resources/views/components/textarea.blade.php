...
<div>
    ...
    <textarea id="{{ $name }}" name="{{ $name }}" @if($hasError) aria-invalid="true" aria-describedby="{{ $name }}-error" @endif {{ $attributes->class('form-control min-h-36 resize-y') }}>{{ $slot }}</textarea>
